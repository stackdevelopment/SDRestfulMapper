...
{	
	...
	//Start of the root object
		...
		//	NSLog(@"Checking Element Name %@", [[self convertElementName:elementName] camelize]);
		//NSLog(@"Checking Property Names Name %@", [[self.parsedObject class] propertyNames]);
        ...
        ([[[self.parsedObject class] propertyNames] containsObject:[[self convertElementName:elementName] camelize]])) {
			...
			//NSLog(@"FOUND matching property! %@", elementName);	
		} else {
			...
			//			NSLog(@"No Matching property found: %@",self.currentPropertyName);
...
{    
	if(self.contentOfCurrentProperty != nil && self.currentPropertyName != nil) {
		...
		//	NSLog(@"setting content: %@  to property %@", self.currentPropertyName, self.contentOfCurrentProperty);
		...
		if ([self.unclosedProperties count] > 0) {
			...
			if ([[[self.unclosedProperties lastObject] objectAtIndex:1] isKindOfClass:[NSArray class]]) {
				...
				NSLog(@"Trying to set element: %@", [self convertElementName:[elementName camelize]]);
