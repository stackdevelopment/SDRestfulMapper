...
+ (id) deserializeJSON:(id)jsonObject {
	...
	if ([jsonObject isKindOfClass:[NSArray class]]) {
		...
		NSLog(@"Class Name = %@", [objectName toClassName]);
		if (objectClass != nil) {
			...
			NSLog(@"Object Class not accepted");
		}
		id properties = [[(NSDictionary *)jsonObject allValues] objectAtIndex:0];
		if ([properties isKindOfClass:[NSDictionary class]]) {
			NSLog(@"Deserializing a dictionary");
			...
			NSLog(@"Deserializing an Array");
			result = [NSMutableArray array];
			for (id childObject in jsonObject) {
				[result addObject:[self deserializeJSON:childObject]];
			}
		}else{
			NSLog(@"Uh, something else");
		}
		//Success to here...
